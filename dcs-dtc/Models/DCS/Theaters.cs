...
namespace DTC.Models.DCS
{
    public class Theater
    {
        ...
        private static AirbaseListItem[] airbaseListItems;

        public static AirbaseListItem[] GetAirbaseListItems()
        {
            var list = new List<AirbaseListItem>();

            foreach (var theater in Theater.Theaters)
            {
                foreach (var ab in theater.Airbases)
                {
                    list.Add(new AirbaseListItem(theater.Name, ab.Name, ab.Latitude, ab.Longitude, ab.Elevation));
                }
            }

            airbaseListItems = list.ToArray();
            return airbaseListItems;
        }
