...
namespace DTC.Models.AH64.Radios
{
    public class RadioChannel
    {
        ...
        private static Regex fmRegex = new Regex(@"^[0]?[0-3][0-9]\.?[0-9]?[0|2|5|7]?$");
        ...
        private bool ValidateFM(string freq)
        {
            if (!fmRegex.IsMatch(freq))
            {
                return false;
            }
            ...
            if (f < new decimal(30.00))
            {
                return false;
            }
            if (f > new decimal(89.00))
            {
                return false;
            }
